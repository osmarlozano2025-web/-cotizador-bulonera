...
function rutaConfiguracionesObtener() {
    ...
    jsonSalida([
        ...
        // V004: mientras esté apagado, los precios se calculan como siempre.
        'motor_descuentos' => [
            'activo' => (bool) ($config['motor_descuentos']['activo'] ?? false),
            'base' => $config['motor_descuentos']['base'] ?? 'lista',
        ],
        // V005: ajuste por familia. Negativo descuenta, positivo aumenta.
        'rango_descuento' => [
            'min' => (float) ($config['rango_descuento']['min'] ?? -50),
            'max' => (float) ($config['rango_descuento']['max'] ?? 50),
        ],
        'ajustes_familia' => [
            'buloneria'  => (float) ($config['ajuste_buloneria']['valor']  ?? 0),
            'tolsen'     => (float) ($config['ajuste_tolsen']['valor']     ?? 0),
            'mechas'     => (float) ($config['ajuste_mechas']['valor']     ?? 0),
            'electrodos' => (float) ($config['ajuste_electrodos']['valor'] ?? 0),
        ],
    ]);
}
