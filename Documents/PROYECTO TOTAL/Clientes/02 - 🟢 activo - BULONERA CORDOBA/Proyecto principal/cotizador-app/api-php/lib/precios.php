 ...
 */
function calcularDesglose(array $item, array $cfg, string $condicionPago, float $descCliente = 0): array
{
    ...
    if (!$cfg['activo']) {
        // Comportamiento histórico: precio de granel menos el descuento del cliente.
        $neto = encadenarDescuentos($granel, [$descCliente]);
        return [
    ...
    return [
        ...
        'descPago2' => $p2,
        'descCliente' => round($descCliente, 2),
        'precioNeto' => round(encadenarDescuentos($base, [$f1, $f2, $p1, $p2, $descCliente]), 2),
        'motorActivo' => true,
    ];
}
